...
export default function SpecialBenefits({ data }) {
  ...
  return (
    <section className="w-full max-w-5xl mx-auto px-2 sm:px-4 md:px-6 pt-0 pb-8 overflow-hidden">
        ...
        {data.cards?.map((card, index) => (
          <div 
            ...
            className={`group w-full border border-pink-200/60 shadow-[0_10px_30px_rgba(219,80,112,0.1)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_15px_35px_rgba(219,80,112,0.18)] ${
              // Card 1 & Card 2: Solid light background colors for mobile to prevent black background bug, gradient for desktop
              index === 0 
          ...
          >
            ...
            <div className="w-full md:pl-3.5">
              <h3 className="text-[11px] sm:text-xs md:text-[15px] font-bold text-[#231F20] leading-tight">
                {card.title}
              </h3>
              <p className="text-[9px] sm:text-[10px] md:text-[11px] text-gray-600 leading-relaxed mt-1 md:mt-0.5">
                {card.description}
              </p>
            </div>
          </div>
